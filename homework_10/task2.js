...
function fighter(commonFighter) {
  let combatHistory = {
      wins: 0,
      loses: 0
  };
  return {
      ...
      fight: function(decider) {
        if (commonFighter.block) {
          return false;
        } else {
          decider.getStats().hp -= commonFighter.attack;
            if (commonFighter.attack >= decider.getStats().hp) {
            combatHistory.wins++;
            decider.getCombatHistory().loses++;
            }
          return true;
        }
      }
